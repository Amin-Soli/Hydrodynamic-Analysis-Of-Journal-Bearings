  ...
  if (CommWorld.MyPID() == 0)
    {
      ...
      fixedPadBearings <F> myFObject(path);
      ...
      InitialGuess[0] = 0.6*((double)rand() / (double)RAND_MAX ) + 0.2;   
      InitialGuess[1] =  1.0;    
     ...
     if (verbose) {
      ...
      RCP<Epetra_CrsMatrix> A = rcp (new Epetra_CrsMatrix (Copy, Map, 2));
      {
		...
		for(int i=0;i<2;i++)
		{
		   ...
		   A.get()->InsertGlobalValues (i, 2, &values[0], &indices[0]);
		}
     ...
     /******************* start algorithm by Trust Region Based Solver  **************************/
      ...
      {		 
         ...
         checkSolution(InitialGuess,flag_check_solution);
         ...
         if( status == NOX::StatusTest::Unconverged && testSolution(InitialGuess,testInitialGuess) )
         {
			 num_correction++ ;
	  ...
	  fixedPadBearings <double> myDoubleObject(path);
